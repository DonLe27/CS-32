...
bool GenomeMatcherImpl::findGenomesWithThisDNA(const string& fragment, int minimumLength, bool exactMatchOnly, vector<DNAMatch>& matches) const
{
	...
	{	
		...
		if (matchLength >= minimumLength)
		{
			...
			newMatch.length = beginMatches[i].length;
...
bool GenomeMatcherImpl::findRelatedGenomes(const Genome& query, int fragmentMatchLength, bool exactMatchOnly, double matchPercentThreshold, vector<GenomeMatch>& results) const
{
	...
	int matchCount; //Holds number of subsequences in query
	bool result = false;
	...
	for (int i = 0; i < query.length() / fragmentMatchLength; i++) //For each subsequence of query
	{
		...
		if (!success) //couldn't extract for some reason
			continue;
		if (findGenomesWithThisDNA(seq, fragmentMatchLength, true, matches)) //Found at least one match
		{
			result = true;
			matchCount++;
		}
		for (int i = 0; i < matches.size(); i++) //For all DNA matches
		{
		...
		for (it = mapMatches.begin(); it != mapMatches.end(); it++) //Transfer mapMatches to results
		{
			...
			double percent = (1.0 * it->second) / matchCount;
